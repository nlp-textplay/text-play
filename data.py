...
def to_var(x, gpu, volatile=False):
    if torch.cuda.is_available():
#        x = x.cuda()
        x=x.to(torch.device('cuda:{}'.format(gpu)))
...
def prepare_data(data_path, max_num, task_type):
    ...
    train_file_list = []
    train_label_list = []
    if task_type == 'yelp' or task_type == 'imdb' or task_type=='amazon':
        train_file_list = [
...
class non_pair_data_loader():
    def __init__(self, batch_size, id_bos, id_eos, id_unk, max_sequence_length, vocab_size, gpu):
    ...
    def create_batches(self, train_file_list, train_label_list, if_shuffle=True):
        ...
        for _index in range(self.num_batch):
            ...
            src = get_cuda(torch.tensor(batch_encoder_input, dtype=torch.long), self.gpu)
            tgt = get_cuda(torch.tensor(batch_decoder_input, dtype=torch.long), self.gpu)
            tgt_y = get_cuda(torch.tensor(batch_decoder_target, dtype=torch.long), self.gpu)
    ...
    def make_std_mask(self, tgt, pad):
        ...
        tgt_mask = (tgt != pad).unsqueeze(-2).to(self.device)
        tgt_mask = tgt_mask & Variable(
            subsequent_mask(tgt.size(-1)).to(self.device).type_as(tgt_mask.data))
        return tgt_mask
